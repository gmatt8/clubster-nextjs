...
export default function ClubInfo({ club }) {
  ...
  // Preload images before updating slider (fix for mobile glitch)
  useEffect(() => {
    const preload = async () => {
      await Promise.all(
        images.map((src) => {
      ...
      <div className="relative w-full h-[350px] md:h-[500px] rounded-xl overflow-hidden">
        <div ref={sliderRef} className="keen-slider h-full overflow-hidden">
          {images.map((src, index) => (
            <div
              key={index}
              className="keen-slider__slide w-full shrink-0 flex items-center justify-center cursor-zoom-in"
  ...
  return (
    <div className="w-full max-w-5xl px-6">
      <div ref={fullscreenRef} className="keen-slider overflow-hidden">
        {images.map((src, i) => (
          <div key={i} className="keen-slider__slide w-full shrink-0 flex items-center justify-center">
            <img
              src={src}
